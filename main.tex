...
\[
    Z(X,t) = \exp \left(\sum_{m=0}^\infty \frac{|X(\mathbb{F}_{q^m})|}{m} t^m \ \right).
\]
...
but varieties in general. The construction takes place in the so called \emph{Grothendieck ring of varieties} $\gring[k]$ over some field $k$, 
which consists of isomorphism classes of varieties subject to some additional relations (for the complete definition see TODO), and is given
as 
\[
    Z_{\mu}(X,t) \coloneqq \sum_{n=0}^{\infty} \mu \left( \left[ \Sym^n(X) \right] \right) t^n
\]
...
Kapranov was able to prove this for smooth projective curves (\cite{kapranov}[TODO]) and conjectured that it is true in general.
%TODO: really start a sentence with BUT?
But it turns out that in this more general setting the zeta function is not as well behaved in terms of rationality as in the classical case:
Larsen and Lunts show in \cite{MR1996804} that the zeta function even fails to be rational for surfaces when working over $\mathbb{C}$.
...
This section culminates in a structure theorem due to Bittner (\cite{Bittner}) which will allow us to simplify the original proof by 
Larsen and Lunts substantially.
...
Construction of the motivic measure $\mu_h$ under wich $Z_{\mu_h}(X,t)$ becomes irrational is content of Section TODO while irrationality
...
$k = k_0(t)$ with $\chara k = p$ and $X = \Spec(k[x]/(x^p - t))$, $Y \coloneqq \Spec(K) \coloneqq \Spec(k(t^{\frac{1}{p}}))$. 
$X$ is a variety since $x^p - t$ is irreducible, hence $X$ is reduced. But the product can be described as
    \[
        X \times Y = \Spec(K[x]/(x^p -t)) = \Spec(K[X]/((x-t^{\frac{1}{p}})^p).
    \]
And hence is not reduced. We might adapt the defintion to this case by changing the defintion of the product to 
$[X][Y] \coloneqq [(X \times Y)_{\text{red}}]$, taking the induced reduced scheme structure on $X \times Y$, but since we are only working
in characteristic zero or over finite fields we will not bother with this technical issue any more.
...
former are TODO , but in the Grothendieck ring we get
\begin{align*}
...
\begin{proof}
    ...
    So let $X$ be an irreducible variety of dimension $n$, by choosing some nonempty affine subvariety and passing to its projective closure we
...
\begin{align*}
    \Psi_h(X)\Psi_h(Y) &= \left( \sum_n h^{i,0}(X)t^i \right) \left( \sum_n h^{i,0}(Y)t^i \right)  
    = \sum_n \left( \sum_{i+j=n} h^{i,0}(X) h^{j,0}(Y) \right )t^n \\ &= \sum_n h^{n,0}(X \times_k Y) = \Psi_h(X \times_k Y)
\end{align*}
...
\begin{proof}
    ...
    Hence 
    $p_0 + \dots + p_et^e = \left(\sum a_it^i\right)\left(q_0 + \dots + q_d t^d\right) = \sum_{i=0}^{\infty} \left(\sum_{k+l=i} a_kq_0\right)t^i$.
...
\begin{proof}[Proof of Theorem \ref{irrational}]
    ...
    \begin{gather*}
            \sum_{\sigma \in S_{n+1}} \sgn(\sigma) \mu_h ( \prod_{i=1}^{n+1} \Sym^{m + i + \sigma(i) - 2}(X))  = 0 \\ 
            \Leftrightarrow
            \mu_h \Big ( \prod_{i=0}^n \Sym^{m + 2i}(X) \Big )  = - \sum_{\substack{\sigma \in S_{n+1} \\ \sigma \neq id}} 
            \sgn(\sigma) \mu_h \Big ( \prod_{i=0}^n \Sym^{m + i + \sigma(i+1) - 1}(X) \Big )
    \end{gather*}
