...
im Kontext des Grothendieckrings der Varietäten $\gring[k]$ über Isomorphieklassen von Varietäten zwischen denen gewissen Relationen bestehen
(Siehe \ref{gringdef} für die genaue Definition). 
...
\begin{theorem}[{\cite[Thm. 1.6]{MR1996804}}]
Sei $k = \mathbb{C}$. Dann existiert ein Körper $\mathcal{H}$ und ein Ringhomomorphismus $\mu_h \colon \gring to \mathcal{H}$ mit der folgenden
Eigenschaft: Falls $X$ eine glatte, projektive, komplexe Fläche mit geometrischem Genus $P_g(X) = h^{2,0}(X)$ mindestens 2, dann ist die
motivische Zetafunktion $\zeta_{\mu_h}(X,t)$ nicht rational.
\end{theorem}
Wir werden ein Theorem von Bittner (\cite{Bittner}) um den Ursprünglichen Beweis deutlich kompakter darstellen zu können.
...
$k = k_0(t)$ with $\mathrm{char} k_0 = p$ and $X = \Spec(k[x]/(x^p - t))$, $Y = \Spec(K) \coloneqq \Spec(k(t^{\frac{1}{p}}))$. 
$X$ is a variety since $x^p - t$ is irreducible in $k_0(t)$. The product, on the other hand, can be described as
    \[
...
We first construct the quotient in the affine case.
\begin{lemma}
    Let $X = \Spec{A}$ be an affine variety with a group $G$ acting on it. Then the categorial quotient exists, is itself a variety and
...
thus proving Theorem \ref{irrational}. We need one technical lemma about the genus of symmetric powers of X, which we cannot prove here.
