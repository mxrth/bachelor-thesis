...
\begin{definition}
    Let $\mu: \gring \to A$ be a motivic measure, $X$ a variety, then we define the zeta function $\zeta_{\mu}(X,t) \in A[[t]]$ as
    \[
...
\begin{example}
    ...
    we calculate
    \begin{align*}
        \zeta_{id}(\mathbb{P}_k^1, t) & = \sum_{n=0}^\infty [{\mathbb{P}_k^1}^{(n)}]t^n \\
...
Kaparnov proves in \cite{Kaparnov} that the zeta function of a curve with coefficents in a field is in fact always rational. 
